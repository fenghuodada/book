...
public class MetaDataStore {
    ...
    private static void safeDeleteCorruptFile(File file) {
        if (file.exists() && file.delete()) {
            ...
            logger.m3344i("Deleted corrupt file: " + file.getAbsolutePath());
        }
    }
    ...
    public Map<String, String> readKeyData(String str, boolean z) {
        ...
        try {
            ...
            e = e3;
            Logger.getLogger().m3339w("Error deserializing user metadata.", e);
            safeDeleteCorruptFile(internalKeysFileForSession);
            CommonUtils.closeOrLog(fileInputStream, "Failed to close user metadata file.");
            return Collections.emptyMap();
        }
    ...
    public String readUserId(String str) {
        ...
        if (!userDataFileForSession.exists() || userDataFileForSession.length() == 0) {
            Logger.getLogger().m3348d("No userId set for session " + str);
        ...
        try {
            try {
                ...
                Logger.getLogger().m3348d("Loaded userId " + jsonToUserId + " for session " + str);
                CommonUtils.closeOrLog(fileInputStream, "Failed to close user metadata file.");
                return jsonToUserId;
            } catch (Throwable th2) {
                th = th2;
        ...
        try {
            try {
                ...
                try {
                    ...
                    Logger.getLogger().m3339w("Error serializing key/value metadata.", e);
                    safeDeleteCorruptFile(internalKeysFileForSession);
        ...
        try {
            try {
                ...
                try {
                    ...
                    e = e;
                    bufferedWriter = bufferedWriter2;
                    Logger.getLogger().m3339w("Error serializing user metadata.", e);
