...
abstract class zzgcp {
    ...
    public zzgcp(byte[] bArr) throws GeneralSecurityException {
        if (!zzgcv.zza(1)) {
    ...
    public abstract zzgcn zza(byte[] bArr, int i) throws InvalidKeyException;
    ...
    public final byte[] zzb(ByteBuffer byteBuffer, byte[] bArr, byte[] bArr2) throws GeneralSecurityException {
        if (byteBuffer.remaining() >= 16) {
            ...
            try {
                ...
                if (!MessageDigest.isEqual(zzgcs.zza(bArr4, order.array()), bArr3)) {
